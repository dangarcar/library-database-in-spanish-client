...
/**
 * Esta clase es la que da la descripci�n avanzada de cada perfil dando los detalles sobre los es.library.databaseinspanish.perfil
...
class ExtendedGUIPerfil extends JPanel{
		...
		if(!contenidosEnPrestamo.isEmpty()) {
			...
			btnDevolver.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(JOptionPane.showConfirmDialog(null, "�Est�s seguro de que quieres devolver el es.library.databaseinspanish.contenido seleccionado?", "Confirmaci�n", JOptionPane.YES_NO_OPTION) == 0) {
						try {
							if(Devolver(c, p)) {
								list.remove(list.getSelectedIndex());
								JOptionPane.showMessageDialog(null, "Gracias por haber devuelto el es.library.databaseinspanish.contenido "+c.getID(),"Operaci�n realizada correctamente",JOptionPane.INFORMATION_MESSAGE);
							} else {
							...
							JOptionPane.showMessageDialog(null, "No se ha podido devolver el es.library.databaseinspanish.contenido, el es.library.databaseinspanish.contenido no est� disponible", "Error", JOptionPane.ERROR_MESSAGE);
						}
					}
			...
			JLabel lblSeSiente = new JLabel("Este perfil no tiene contenidos actualmente en pr�stamo");
	...
	public String getExtendedDescripcion() {
		return "<html><body><p style = \"font-family:Segoe UI,Frutiger,Frutiger Linotype,Dejavu Sans,Helvetica Neue,Arial,sans-serif;\">"+
				...
				"<br><b>Correo Electr�nico: </b>"+p.getCorreoElectronico()+
				"<br><b>Direcci�n: </b>"+p.getDireccionDeCasa()+
				"<br><b>Administrador: </b>"+((p instanceof Admin)? "S�":"No")+
				"</p></body></html>";
	}
...
class ButtonToAdminOrPerfil extends JButton implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if(p instanceof Admin) {
			try {
				if(JOptionPane.showConfirmDialog(null, "�Desea conventir a "+p.getDNI()+" en perfil normal?", "Confirmaci�n", JOptionPane.YES_NO_OPTION) == 0) {
			...
			try {
				if(JOptionPane.showConfirmDialog(null, "�Desea conventir a "+p.getDNI()+" en administrador?", "Confirmaci�n", JOptionPane.YES_NO_OPTION) == 0) {
					PerfilToAdmin(p);
