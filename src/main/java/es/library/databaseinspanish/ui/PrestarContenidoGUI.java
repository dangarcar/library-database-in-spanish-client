...
/**
 * Esta clase es la es.library.databaseinspanish.ui para coger prestado un libro pasado por par�metro
 * @author danie
 *
 */
public class PrestarContenidoGUI extends JFrame{
	...
	public PrestarContenidoGUI(Contenido c) {
		...
		setTitle("Prestar es.library.databaseinspanish.contenido");
		...
		btnConfirmar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "No se ha reconocido un n�mero en el campo \"ID\"", "Error", JOptionPane.WARNING_MESSAGE, new ImageIcon("src/main/resources/files/images/error.png"));
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "Ha habido un error la identificaci�n de usuario", "Error", JOptionPane.WARNING_MESSAGE, new ImageIcon("src/main/resources/files/images/error.png"));
					textField.setBorder(new LineBorder(Color.RED,3));
				}
				
				if(perfil != null && dni != 0) {
					try {
						int opt = JOptionPane.showConfirmDialog(null, "�Confirma que quiere que "+perfil.getDNI()+" coja prestado el es.library.databaseinspanish.contenido "+c.getID()+"?", "Confirmaci�n",JOptionPane.YES_NO_OPTION);
						if(opt == 0) {
							Prestar(c, perfil);
							if(guardarRecibo.isSelected()) new Recibo(((Component)e.getSource()).getParent().getParent(),c,perfil);
							setVisible(false);
							JOptionPane.showMessageDialog(null, "Felicidades por haber cogido prestado el es.library.databaseinspanish.contenido "+c.getID(),"Felicidades",JOptionPane.INFORMATION_MESSAGE);
							return;
						}
					} catch (ExcepcionContenido | ExcepcionPerfil e1) {
						e1.printStackTrace();
						JOptionPane.showMessageDialog(null, "Ha habido un error con el pr�stamo del es.library.databaseinspanish.contenido", "Error", JOptionPane.WARNING_MESSAGE, new ImageIcon("src/main/resources/files/images/error.png"));
					}
				}
	...
	public String getDescripcion() {
		return "<html><body><p>"+
				"<FONT FACE=\"Segoe UI\" SIZE=5 COLOR=\"black\"<b>T�tulo: </b>"+c.getTitulo()+"</FONT>"+
	...
	public File createRecibo() {
		...
		try {
			text = "Elementos prestados el "+(new SimpleDateFormat("dd/MM/yyyy hh:mm")).format(new Date())+"\nA "+p.getApellido()+", "+p.getNombre()+"\n\n"+
					"T�tulo del elemento: "+c.getTitulo()+"\n"+
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Ha habido un error con el recibo del pr�stamo", "Error", JOptionPane.WARNING_MESSAGE, new ImageIcon("src/main/resources/files/images/error.png"));
		} finally {
			if(d != null) {
				d.close();
			}
