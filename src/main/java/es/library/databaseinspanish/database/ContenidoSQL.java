...
public class ContenidoSQL {
	...
	/**
	 * Este método consigue un es.library.databaseinspanish.contenido de la BD según su ID absoluto de la biblioteca
	...
	/**
	 * Este método guarda un es.library.databaseinspanish.contenido audiovisual pasado por parámetro en la base de datos
	...
	public static void WriteAudiovisual(Audio audiovisual) throws ExcepcionContenido {
		...
		if (audiovisual != null) {
			try{
				...
				throw new ExcepcionContenido("Ocurrió un error. Es posible que este es.library.databaseinspanish.contenido audiovisual ya exsta en la base de datos",audiovisual);
			} finally {
				//Si la conexión a la base de datos existe, la cierro
				if(conector != null) {
					conector.cerrar();
			...
			throw new ExcepcionContenido("El es.library.databaseinspanish.contenido audiovisual seleccionado no es válido",audiovisual);
		}
	}
	...
	public static boolean Prestar(Contenido c,Perfil p) throws ExcepcionDisponibilidad, ExcepcionContenido, ExcepcionPerfil {
		...
		if (c != null && p != null) {
			try{
				...
				//Compruebo que el es.library.databaseinspanish.contenido esté disponible en la base de datos y en el objeto
				...
				if(t && c.getDisponibilidad()) {
					...
					//Modifico tambien la base de datos de los es.library.databaseinspanish.perfil añadiendo el es.library.databaseinspanish.contenido a la lista de prestados del perfil pasado por parámetro
	...
	public static boolean Devolver(Contenido c, Perfil p) throws ExcepcionDisponibilidad, ExcepcionPerfil {
		...
		if(c != null && p != null) {
			try {
				...
				//Compruebo que el es.library.databaseinspanish.contenido no esté disponible en la base de datos y en el objeto
				st = connect.prepareStatement("SELECT Disponible FROM Contenidos WHERE ID = ?");
				st.setInt(1, c.getID());
				ResultSet rs = st.executeQuery();
				st.clearParameters();
				//Compruebo que el es.library.databaseinspanish.contenido no está disponible ni en la base de datos ni en el objeto en sí
				t = !(rs.getBoolean("Disponible"));
				
				//Si el es.library.databaseinspanish.contenido no está disponible y el perfil que devuelve es propietario actualmente de ese es.library.databaseinspanish.contenido
